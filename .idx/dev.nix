{pkgs, ...}:
{
  ...
  packages = [
    ...
    pkgs.bun
    pkgs.zip
    pkgs.curl # Corrected typo from curlenv
    pkgs.flutter # Moved from env block
    pkgs.cmake # Moved from env block
  ];
  env = {
  ...
  idx.workspace = {
    ...
    onStart = {
      # Moved postgresRun command here from the invalid 'processes' block
      postgresRun = "postgres -D ./local -k /tmp"; 
      genkit-start = ''
        cd genkit
        npm install
